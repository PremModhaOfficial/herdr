...
pub(crate) const JCODE_CONFIG_DIR_ENV_VAR: &str = "JCODE_CONFIG_DIR";
...
pub(crate) fn jcode_dir() -> io::Result<PathBuf> {
    // JCODE_CONFIG_DIR is a herdr-level override only; jcode itself resolves
    // its config home to ~/.jcode.
    config_dir_from_env_or_home(JCODE_CONFIG_DIR_ENV_VAR, &[".jcode"])
}
